...
def identify_barcode_format(barcode: str) -> Optional[Tuple[str, str]]:
    ...
    # EAN-13 - Check first as it's common for OTC products
    if length == 13:
        # Check if it's a German pharmaceutical EAN (NTIN) (starts with 4150)
        if barcode.startswith("4150"):
            # Extract the 8-digit PZN from positions 4-11
            pzn = barcode[4:12]
...
def validate_ean13(ean: str) -> bool:
    """
    Validate EAN-13 barcode with standard check digit.
    EAN-13: 13 digits total (12 digits + 1 check digit)
    """
    if not ean or len(ean) != 13 or not ean.isdigit():
        return False

    # Calculate check digit using standard EAN algorithm
    total = 0
