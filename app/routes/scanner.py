...
from models import db, MedicationPackage, ProductPackage, ScannedItem, PackageInventory
...
def scan():
    ...
    existing_scanned = ScannedItem.query.filter_by(serial_number=parsed['serial']).first()
    if existing_scanned:
        # Check if it's in active inventory
        active_inventory = PackageInventory.query.filter_by(
    ...
    if parsed.get('gtin'):
        product_package = ProductPackage.query.filter_by(gtin=parsed['gtin']).first()
    ...
    if product_package:
        ...
        if not product_package.national_number and parsed.get('national_number'):
            product_package.national_number = parsed['national_number']
            product_package.national_number_type = parsed.get('national_number_type')
            updated = True
        ...
        if product_package.product.legacy_medication_id:
            ...
            pending_order_item = (
                ...
                .filter(
                    OrderItem.medication_id == product_package.product.legacy_medication_id,
            ...
            if pending_order_item:
                units_still_needed = pending_order_item.quantity_needed - pending_order_item.units_received
                pending_order_item.units_received += product_package.quantity
                
                # Check if this fulfills or overfills the order
                if pending_order_item.units_received >= pending_order_item.quantity_needed:
                    pending_order_item.fulfillment_status = 'fulfilled'
                    pending_order_item.fulfilled_at = datetime.utcnow()
                    if pending_order_item.units_received > pending_order_item.quantity_needed:
                        overage = pending_order_item.units_received - pending_order_item.quantity_needed
                        fulfillment_message = f"Order fulfilled with {overage} extra units (substitution)"
                        pending_order_item.fulfillment_notes = f"Fulfilled with package substitution. Received {product_package.quantity} units in {product_package.package_size}. Overage: {overage} units."
                    else:
                        fulfillment_message = "Order fulfilled exactly"
                else:
                    pending_order_item.fulfillment_status = 'partial'
                    remaining = pending_order_item.quantity_needed - pending_order_item.units_received
                    fulfillment_message = f"Partial fulfillment: {remaining} units still needed"
            ...
            if inventory:
                ...
                old_total = medication.total_inventory_count - product_package.quantity
            ...
            response_data = {
                ...
                'package_info': {
                    ...
                    'manufacturer': product_package.manufacturer or product_package.product.manufacturer
                },
                'parsed_data': {
            ...
            return jsonify({
                'error': _('Product found but not linked to inventory system'),
                'hint': _('This product needs to be linked to a medication record for inventory tracking'),
                'product_info': {
            ...
            if pending_order_item:
                ...
                if pending_order_item.units_received >= pending_order_item.quantity_needed:
                    ...
                    if pending_order_item.units_received > pending_order_item.quantity_needed:
                        overage = pending_order_item.units_received - pending_order_item.quantity_needed
                        fulfillment_message = f"Order fulfilled with {overage} extra units (substitution)"
                        pending_order_item.fulfillment_notes = f"Fulfilled with package substitution. Received {quantity} units in {package.package_size}. Overage: {overage} units."
                    else:
                        fulfillment_message = "Order fulfilled exactly"
                else:
                    pending_order_item.fulfillment_status = 'partial'
