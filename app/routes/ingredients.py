...
from models import db, ActiveIngredient, MedicationProduct, ProductPackage, Medication
...
def index():
    ...
    for ingredient in ingredients:
        products = MedicationProduct.query.filter_by(active_ingredient_id=ingredient.id).all()
...
def show_product(id: int):
    ...
    if product.legacy_medication:
        ...
        package_inventory = (
            ...
            .filter(PackageInventory.medication_id == product.legacy_medication_id)
...
def edit(id: int):
    ...
    if request.method == "POST":
        ...
        ingredient.strength = request.form.get("strength", "").strip() or None
        ingredient.strength_unit = request.form.get("strength_unit", "").strip() or None
...
def delete(id: int):
    ...
    product_count = MedicationProduct.query.filter_by(active_ingredient_id=id).count()
    if product_count > 0:
        flash(_("Cannot delete active ingredient with existing products. Please delete all products first."), "error")
...
def delete_product(id: int):
    ...
    if product.legacy_medication_id:
        ...
        inventory_count = PackageInventory.query.filter_by(
            medication_id=product.legacy_medication_id
        ).filter(PackageInventory.status.in_(['sealed', 'open'])).count()
        if inventory_count > 0:
            flash(_("Cannot delete product with existing inventory. Please deplete inventory first."), "error")
    ...
    if was_default:
        ...
        if remaining_products:
            ...
            flash(_("Default product updated to %(name)s", name=remaining_products.brand_name), "info")
        else:
...
def new_product():
    ...
    if request.method == "POST":
        ...
        if ingredient_id.startswith("new:"):
            ...
            strength_unit = request.form.get("strength_unit", "").strip() or None
            form = request.form.get("form", "").strip() or None
            
            # Check if it already exists with same name, strength, and form
            existing_query = ActiveIngredient.query.filter_by(name=ingredient_name)
            if strength:
                existing_query = existing_query.filter_by(strength=strength, strength_unit=strength_unit)
            if form:
        ...
        if return_to_ingredient:
            return redirect(url_for("ingredients.show", id=return_to_ingredient))
        return redirect(url_for("ingredients.show_product", id=product.id))
...
def product_packages(id: int):
    ...
    product = MedicationProduct.query.get_or_404(id)
    packages = ProductPackage.query.filter_by(product_id=id).order_by(ProductPackage.package_size).all()
...
@ingredients_bp.route("/products/<int:id>/packages/new", methods=["GET", "POST"])
def new_package(id: int):
    ...
    if request.method == "POST":
        package = ProductPackage(
            ...
            national_number=request.form.get("national_number", "").strip() or None,
            national_number_type=request.form.get("national_number_type", "").strip() or None,
...
def edit_package(id: int):
    ...
    if request.method == "POST":
        ...
        package.national_number = request.form.get("national_number", "").strip() or None
        package.national_number_type = request.form.get("national_number_type", "").strip() or None
        package.manufacturer = request.form.get("manufacturer", "").strip() or None
        package.list_price = float(request.form.get("list_price") or 0) or None
        
        db.session.commit()
        flash(_("Package updated successfully"), "success")
        return redirect(url_for("ingredients.show_product", id=package.product_id))
...
@ingredients_bp.route("/<int:ingredient_id>/set-default-product/<int:product_id>", methods=["POST"])
...
@ingredients_bp.route("/<int:ingredient_id>/clear-default-product", methods=["POST"])
...
def migrate_medication(medication_id: int):
    ...
    existing = MedicationProduct.query.filter_by(legacy_medication_id=medication_id).first()
    if existing:
