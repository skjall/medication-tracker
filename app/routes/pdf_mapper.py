...
from models import db, PDFTemplate, Medication, MedicationProduct, ActiveIngredient
...
def resolve_pdf_path(template):
    ...
    data_dir = Path(current_app.config.get('DATABASE_PATH', current_app.root_path)).parent
    
    # Try various locations where the file might be
    paths_to_try = [
        # Try the stored path as-is
        Path(template.file_path) if os.path.isabs(template.file_path) else None,
        # Try in persistent data directory (preferred location)
        data_dir / 'pdf_templates' / filename,
        # Try relative to app root
        Path(current_app.root_path) / template.file_path,
        # Try in static/pdf_templates with just the filename (legacy)
        Path(current_app.root_path) / 'static' / 'pdf_templates' / filename,
        # Docker-specific paths
        Path('/app') / template.file_path if not template.file_path.startswith('/') else None,
        Path('/app/static/pdf_templates') / filename,
        # Try without nested app directory (for Docker mounted volumes)
        Path('/static/pdf_templates') / filename,
        # Docker data volume paths
        Path('/data/pdf_templates') / filename,
    ]
    ...
    raise FileNotFoundError(
        f"PDF file '{filename}' not found. Tried paths: {', '.join(tried_paths[:3])}"
    )
...
def index():
    ...
    templates = PDFTemplate.query.filter_by(is_active=True).order_by(PDFTemplate.created_at.desc()).all()
...
def new_template():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            flash(_('PDF template uploaded successfully. Please map the PDF form fields to the table structure.'), 'success')
            return redirect(url_for('pdf_mapper.structure_view', id=template.id))
        else:
...
def structure_view(id):
    ...
    if template.table_config and 'fields' in template.table_config:
        detected_fields = template.table_config['fields']
    
    # Define column headers for the table
    column_headers = [
        {'id': 'medication_name', 'label': _('Medication Name')},
        {'id': 'active_ingredient', 'label': _('Active Ingredient')},
        {'id': 'strength', 'label': _('Strength')},
        {'id': 'unit', 'label': _('Unit')},
        {'id': 'dosage_form', 'label': _('Dosage Form')},
        {'id': 'package', 'label': _('Package')},
        {'id': 'quantity', 'label': _('Quantity')}
    ]
...
def content_view(id):
    ...
    available_fields = {
        'product': [
            ...
            {'id': 'display_name', 'label': _('Full Product Name'), 'icon': '📝'},
        ],
        'ingredient': [
            {'id': 'active_ingredient', 'label': _('Active Ingredient'), 'icon': '💊'},
            {'id': 'strength', 'label': _('Strength'), 'icon': '💪'},
            {'id': 'unit', 'label': _('Unit'), 'icon': '📏'},
        ],
        'dosage': [
            {'id': 'daily_units', 'label': _('Daily Units'), 'icon': '🔢'},
            {'id': 'dosage_form', 'label': _('Dosage Form (tablets, etc.)'), 'icon': '💊'},
            {'id': 'strength_value', 'label': _('Strength Value'), 'icon': '💪'},
            {'id': 'strength_unit', 'label': _('Strength Unit (mg, ml, etc.)'), 'icon': '📏'},
            {'id': 'daily_dosage', 'label': _('Daily Dosage (Strength × Units)'), 'icon': '➕'},
        ],
        'package': [
            {'id': 'package_size', 'label': _('Package Size'), 'icon': '📦'},
            {'id': 'quantity', 'label': _('Quantity per Package'), 'icon': '🔢'},
            {'id': 'pzn', 'label': _('PZN'), 'icon': '🔖'},
            {'id': 'gtin', 'label': _('GTIN'), 'icon': '📊'},
        ],
        'order': [
            {'id': 'packages_ordered', 'label': _('Number of Packages'), 'icon': '📦'},
            {'id': 'package_size_ordered', 'label': _('Package Size (N1/N2/N3)'), 'icon': '📏'},
...
def detect_fields(id):
    ...
    try:
        ...
        if reader.get_form_text_fields():
            ...
            return jsonify({
                'success': True,
                'message': _('Found %(count)s existing form fields in PDF', count=len(existing_fields)),
                'fields': existing_fields,
                'existing_fields': True,
                'fields_count': len(existing_fields)
            })
        ...
        return jsonify({
            'success': False,
            'message': _('No form fields found in PDF. Would you like to create a grid of fields?'),
...
def create_grid_fields(id):
    ...
    try:
        ...
        temp_pdf_path = pdf_path.parent / f"temp_{template.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        if success:
            ...
            return jsonify({
                'success': True,
                'message': _('Created %(count)s form fields in a %(rows)sx%(cols)s grid', 
                           count=len(fields), rows=template.rows_per_page, cols=template.columns_count),
...
def save_mappings(id):
    ...
    return jsonify({
        'success': True,
        'message': _('Mappings saved successfully')
    })
...
def preview_template(id):
    ...
    for med in medications:
        ...
        if product and product.active_ingredient:
            strength_value = str(product.active_ingredient.strength) if product.active_ingredient.strength else str(med.dosage) if med.dosage else ''
            strength_unit = product.active_ingredient.strength_unit if product.active_ingredient.strength_unit else ''
        else:
            # Parse from medication dosage if available (e.g., "400" from "400mg")
            if med.dosage:
                strength_value = str(med.dosage)
            # Try to extract unit from medication name or form field if available
            strength_unit = med.form if hasattr(med, 'form') and med.form else 'mg'
        
        # Calculate daily dosage (strength × daily units)
        daily_dosage = ''
        if strength_value and daily_units:
            try:
                strength_num = float(strength_value)
                total_dosage = strength_num * daily_units
                daily_dosage = f"{total_dosage:.0f} {strength_unit}" if strength_unit else f"{total_dosage:.0f}"
            except (ValueError, TypeError):
                daily_dosage = f"{daily_units} × {strength_value} {strength_unit}".strip()
        
        # Determine dosage form
        dosage_form = 'tablets'  # Default
        if strength_unit:
            if 'ml' in strength_unit.lower():
                dosage_form = 'ml'
            elif 'mg' in strength_unit.lower():
                dosage_form = 'tablets'
            elif 'drops' in med.name.lower():
                dosage_form = 'drops'
        ...
        quantity_per_package = med.package_size_n2 if package_size_ordered == 'N2' else (
            med.package_size_n3 if package_size_ordered == 'N3' else med.package_size_n1
        )
        total_units = packages_ordered * (quantity_per_package or 50)  # Default to 50 if not set
        ...
        med_data = {
            ...
            'unit': product.active_ingredient.strength_unit if product and product.active_ingredient else '',
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'preview_url': url_for('pdf_mapper.download_preview', id=template.id)
        })
...
def generate_pdf(id):
    ...
    medications = Medication.query.filter(Medication.id.in_(medication_ids)).all()
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'download_url': url_for('pdf_mapper.download_generated', id=template.id)
        })
...
def generate_filled_pdf_from_template(template, medications):
    ...
    output_path = Path(tempfile.gettempdir()) / f"generated_{template.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    
    with open(pdf_path, 'rb') as pdf_file:
        pdf_reader = PdfReader(pdf_file)
        pdf_writer = PdfWriter()
        
        # Process medications in batches according to rows per page
        for page_num, page in enumerate(pdf_reader.pages):
            # Calculate which medications go on this page
            start_idx = page_num * template.rows_per_page
            end_idx = min(start_idx + template.rows_per_page, len(medications))
            page_medications = medications[start_idx:end_idx]
            
            if not page_medications and page_num > 0:
                break  # No more medications to process
            ...
            for row_idx, med in enumerate(page_medications, 1):
                ...
                for col_idx in range(1, template.columns_count + 1):
                    ...
                    if formula:
                        ...
                        # Set the field value in the PDF
                        # This would require proper form field handling
